# Routine-Orb System（套路挂球系统，V4.0 MVP）

本文件描述 V4.0 “Single Driver Routine-Orb MVP（单驾驶员·套路球最小可玩闭环）”的机制范围、术语与可观察性验收口径。实现与测试为准：`src/core/*` + `tests/*` + Debug 面板的一键 Run。

## 术语（中英对照）

| 英文 | 中文建议 | 含义 |
| --- | --- | --- |
| Routine | 武功套路 | 一条“技能路线/套路”的标识（当前只有 FireRoutine） |
| Routine Skill | 套路技能 | Routine 里的具体技能（当前映射到 Art1/2/3） |
| RoutineTile | 套路牌 | 套路技能命中后生成的记录项（最多 3 张） |
